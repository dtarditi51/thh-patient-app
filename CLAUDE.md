...
- **Vercel production deploy** (live 2026-05-24, custom domain `hearthousenj.app` provisioned 2026-05-25): https://hearthousenj.app (canonical, use for QR / OG / printed materials) ⟶ underlying deploy at https://thh-patient-app.vercel.app. Project is `drdanieltarditi-4799s-projects/thh-patient-app`, currently personal-account scope (Hobby). **Vercel IS connected to `dtarditi51/thh-patient-app` — pushing `main` auto-deploys to production** (corrected 2026-08-01; this line previously claimed no connection, which was wrong and cost a diagnostic pass. `vercel project inspect` prints no git section either way, so it cannot answer this question — check `githubDeployment` in the deployments API, or run `vercel git connect`, which reports "already connected"). Prefer `git push` over `npx vercel --prod`: the CLI path races the git build and produces duplicate production deploys off one commit. Reserve CLI deploys for shipping uncommitted work. Env vars: Production has 21, Preview has 19. `FIREBASE_SERVICE_ACCOUNT` and `PUSH_ADMIN_TOKEN` are **withheld from Preview deliberately** — `KV_REST_API_*` is already shared, so a preview holding those two could fan out real notifications to real patients via `/api/push/send`; that route is fail-closed without the token. The old Vercel CLI v54.4.1 Preview-env bug is fixed as of v58.4.4 (pipe the value on stdin). Deployment Protection is on by default; preview URLs require Vercel SSO, production URL is public.
